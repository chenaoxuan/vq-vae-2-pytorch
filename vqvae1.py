...
class VQVAE1(nn.Module):
    def __init__(
        ...
        """
        n_embed: Number of vectors in codebook
        embed_dim: Dimension of each vector in codebook

        The config in CogView: channel=512, n_res_block=0, n_res_channel=32, embed_dim=256, n_embed=8192, stride=6
        """
        super().__init__()
        if channel == 2048:
            n_res_block = 0
